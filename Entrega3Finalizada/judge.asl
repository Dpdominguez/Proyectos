...
obstacles(5). //Numero de obstaculos a generar
...
celdasInicio(player1,10).
celdasInicio(player2,10).
...
//modificado para et3(gestion del tercer nivel) hay que acabar de retocarlo
		
//Comprobacion del ganador de la partida


+!checkFinalWinner(W1,W2,W3) : W1=W2 & W2=W3<- -+finalWinner(W1). //Gano los tres niveles

+!checkFinalWinner(W1,W2,W3) : W1=W2 & not(W1=W3) <- -+finalWinner(W1).//Que haya ganado  dos niveles

+!checkFinalWinner(W1,W2,W3) : W1=W3 & not(W1=W2)<- -+finalWinner(W1). //Que haya ganado  dos niveles

+!checkFinalWinner(W1,W2,W3) : W2=W3 & not(W2=W1)<- -+finalWinner(W2). //Que haya ganado  dos niveles

//empates y otros casos, el que tenga mas ountos totales
...
//Ha sido necesario modificar el numero m�ximo de jugadas por jugador dado que var�a segun el nivel
+!comienzoTurno :level(L)& turnoActual(P) & jugadasRestantes(N) & N>0 & jugadasPlayer(P,J)& maxJugadas(L,NumJugadas) & J<NumJugadas <-
	.print("Turno de: ",P,"!");
	-+turnoActivado(1);
...
+!comienzoTurno :level(L)&  jugadasRestantes(N)& turnoActual(P) & jugadasPlayer(P,J) & maxJugadas(L,NumJugadas) & J>=NumJugadas <- .print("FIN DE LA PARTIDA: ",P," ha realizado el maximo de jugadas por jugador").
...
+moverDesdeEnDireccion(pos(X,Y),Dir)[source(P)] :
	turnoActual(P) & not movimientoValido(pos(X,Y),Dir) & turnoActivado(1)<-
			.print("Ha sido invalido",X,Y,Dir);
			-+turnoActivado(0);
			-moverDesdeEnDireccion(pos(X,Y),Dir)[source(P)];
...
+movimientoInvalido(pos(X,Y),Dir,P) :
	not colorFichasDistintos(pos(X,Y),Dir) <-
		-movimientoInvalido(pos(X,Y),Dir,P);
		.print("Movimiento Invalido. Has intentado  intercambiar dos fichas del mismo color",X,Y,Dir);
		-+turnoActivado(1);
...
+startGame : size(N) <- //Este plan se ejecutará cuando el entorno le avise al terminar la generacion del tablero
				...
				.print("                ----- ----- ----- NIVEL 1 ----- ----- -----\n");
				.send(player1,tell,level(1));
				.send(player2,tell,level(1));
				!comienzoTurno.
...
////////////////////////////////////////////////////////////////////
//modificado el numero maximo de jugadas por jugador, ahora cambia segun el nivel
//Finalizacion de un turno
+turnoTerminado(Player) : level(L) & L=1 &( (limitPoints(1,LimitPoints)  & points(L,Player,Points) & Points>=LimitPoints) | (jugadasRestantes(N) & N=0) |  (maxJugadas(L,NumJugadas)&jugadasPlayer(P,J) & J>=NumJugadas)) <- //Final del Nivel 1
...
+turnoTerminado(Player) : level(L) & L=2 & ((limitPoints(2,LimitPoints)  & points(L,Player,Points) & Points>=LimitPoints) | (jugadasRestantes(N) & N=0) |  (maxJugadas(L,NumJugadas)&jugadasPlayer(P,J) & J>=NumJugadas) )<- //Final del Nivel 2
					...
					.print("                      ----- ----- ----- NIVEL 3 ----- ----- -----\n");
					.send(player1,untell,level(2));
					.send(player2,untell,level(2));
					.send(player1,tell,level(3));
					.send(player2,tell,level(3));
					.wait(1000);
					-+level(3);
					-+turnoActual(player1);
					-+jugadasRestantes(40);
					...
					-+obstacles(N/2);
					!generateObstacles;
					!generateDuenhoInicios;
					 repaint;
					!updatePlayersTableroBB.


+turnoTerminado(Player) : level(L) & L=3 &( ((points(L,Player,Points) &Points>=LimitPoints) & (territorio(Player,Numceldas)& size(Tamanho)& obstacles(Obstaculos)& (Numceldas=(Tamanho*Tamanho)-Obstaculos))) | (jugadasRestantes(N) & N=0)|  (maxJugadas(L,NumJugadas)&jugadasPlayer(P,J) & J>=NumJugadas) )<- //Final del Nivel 3
					.print("Nivel 3 finalizado");
					!showPoints;
					!checkLevel3Winner;
					...
					?levelWinner(3,Winner);
					?territorio(Winner,Numceldas)
					.print("[ GANADOR NIVEL 3: ",Winner," celdas conquistadas: ",Numceldas," ] ");					
					-+endGame(1).
...
/////////////////////////////////////
//Anhadido para gestion nivel 3				
+!checkLevel3Winner : level(L) & territorio(player1,P1) & territorio(player2,P2) &  levelWinner(P1,P2,Winner) <-
				-levelWinner(L,_);
				+levelWinner(L,Winner).
...
/////////////////////////////////////////////////////////Hay que retocarlo
//Generacion de celdas iniciales con duenho

+!generateDuenhoInicios:(celdasInicio(player1,Celdas) & Celdas>0) | (celdasInicio(player2,Celdas2) & Celdas2>0) <- //le cambia la propiedad de la celda para que tengan territorio inicial
							.random(X,10);
							.random(Y,10);
							!generateDuenhoInicio(math.round(9*X),math.round(9*Y));
							!generateDuenhoInicios.
					
					
+!generateDuenhoInicio(X,Y):celdasInicio(player1,Celdas) & Celdas>0 & tablero(celda(X,Y,0),ficha(C,T))<-
							-tablero(celda(X,Y,0),ficha(C,T));
							+tablero(celda(X,Y,1),ficha(C,T));
							-celdasInicio(player1,Celdas);
							+celdasInicio(player1,Celdas-1);
							putDuenho(X,Y,1);
							.

+!generateDuenhoInicio(X,Y):celdasInicio(player2,Celdas) & Celdas>0 & tablero(celda(X,Y,0),ficha(C,T))<-
							-tablero(celda(X,Y,0),ficha(C,T));
							+tablero(celda(X,Y,2),ficha(C,T));
							-celdasInicio(player2,Celdas);
							+celdasInicio(player2,Celdas-1);
							putDuenho(X,Y,2);
							.

+!generateDuenhoInicio(X,Y).
...
//RETOCAR PARA ENTREGA3
+intercambiarFichas(X1,Y1,Dir,P) : nextPosition(X1,Y1,Dir,X2,Y2) & plNumb(P,PlNumb) <-
								-+direction(Dir);
								-tablero(celda(X1,Y1,Duenho),ficha(C1,TipoFicha1));
								-tablero(celda(X2,Y2,Duenho2),ficha(C2,TipoFicha2));
								+tablero(celda(X2,Y2,Duenho2),ficha(C1,TipoFicha1));
								+tablero(celda(X1,Y1,Duenho),ficha(C2,TipoFicha2));
								...
								-+explosionFlag(1);
								-+dualExplosionFlag(0);
								-+posicionesIntercambiadas(X1,Y1,X2,Y2,Duenho);
								...
								!compruebaTerritorios. //comprobacion del territorio de cada jugador
								
+!exchangedPatternMatch : posicionesIntercambiadas(X1,Y1,X2,Y2,_) <- 
...
+!patternMatchPosition(I,J): tablero(celda(I,J,_),ficha(C,_)) & comprobarPatrones(C,I,J,StartsAtX,StartsAtY,Direction,Pattern) & not Pattern = "none" & posicionesIntercambiadas(X1,Y1,X2,Y2,Duenho)<-
...
+!fallAndRoll(X,Y) : emptyUnder(X,Y) & tablero(celda(X,Y,_),ficha(Color,Tipo))<-
...
+!fallAndRoll(X,Y) : emptyLeft(X,Y) & tablero(celda(X,Y,_),ficha(Color,Tipo))<- //Se ha elegido que las fichas rueden hacia la izquierda para funcionar correctamente con el orden de exploracion del tablero establecido.
...
+!handlePattern(Color,StartsAtX,StartsAtY,Direction,Pattern) : Pattern = "3inLineH" <-
		delete(Color,StartsAtX,StartsAtY,0);
		delete(Color,StartsAtX,StartsAtY+1,0);
		delete(Color,StartsAtX,StartsAtY+2,0);
...
+!handlePattern(Color,StartsAtX,StartsAtY,Direction,Pattern) : Pattern = "3inLineW" <-
		delete(Color,StartsAtX,StartsAtY,0);
		delete(Color,StartsAtX+1,StartsAtY,0);
		delete(Color,StartsAtX+2,StartsAtY,0);
...
+!handlePattern(Color,StartsAtX,StartsAtY,Direction,Pattern) : Pattern = "4inLineH" <-
		...
		delete(Color,StartsAtX,StartsAtY+1,0);
		delete(Color,StartsAtX,StartsAtY+2,0);
		delete(Color,StartsAtX,StartsAtY+3,0);
...
+!handlePattern(Color,StartsAtX,StartsAtY,Direction,Pattern) : Pattern = "4inLineW" <-
		...
		delete(Color,StartsAtX+1,StartsAtY,0);
		delete(Color,StartsAtX+2,StartsAtY,0);
		delete(Color,StartsAtX+3,StartsAtY,0);
...
+!handlePattern(Color,StartsAtX,StartsAtY,Direction,Pattern) : Pattern = "5inLineH"<-
		...
		delete(Color,StartsAtX,StartsAtY+2,0);
		delete(Color,StartsAtX,StartsAtY+3,0);
		delete(Color,StartsAtX,StartsAtY+4,0);
...
+!handlePattern(Color,StartsAtX,StartsAtY,Direction,Pattern) : Pattern = "5inLineW" <-
		...
		delete(Color,StartsAtX+2,StartsAtY,0);
		delete(Color,StartsAtX+3,StartsAtY,0);
		delete(Color,StartsAtX+4,StartsAtY,0);
...
+!handlePattern(Color,StartsAtX,StartsAtY,Direction,Pattern) : Pattern = "Square" <-
		...
		delete(Color,StartsAtX+1,StartsAtY,0);
		delete(Color,StartsAtX,StartsAtY+1,0);
		delete(Color,StartsAtX+1,StartsAtY+1,0);
...
+!setMovedSteak(X,Y,Pattern) : explosionFlag(1) & tablero(celda(X,Y,_),e) & specialSteak(Pattern,Type)<-
			...
			-tablero(celda(X,Y,Duenho),_);
			+tablero(celda(X,Y,Duenho),m);
...
+!generateSpecialSteak(Pattern,Color) : explosionFlag(1) & tablero(celda(X,Y,_),m) & specialSteak(Pattern,Type)<- //La explosion se produjo directamente por el movimiento del jugador, y la ficha especial se debe de colocar en la ficha movida.
			put(Color,X,Y,Type);
			-tablero(celda(X,Y,Duenho),m);
			+tablero(celda(X,Y,Duenho),ficha(Color,Type));
...
+!dualExplosion : dualExplosionFlag(F) & F=1 & tablero(celda(NX,NY,_),m)<- //Gestion de la explosion de dos patrones contiguos para evitar problemas en la generacion de fichas especiales
			-tablero(celda(NX,NY,Duenho),m);
			+tablero(celda(NX,NY,Duenho),e);
			-+dualExplosionFlag(0).
+!dualExplosion.
...
//Anhadido la gestion del territorio aqui
+!explosion(X,Y) : tablero(celda(X,Y,_),ficha(C,T)) & direction(D) & turnoActual(A) & level(L) & points(L,A,P)& posicionesIntercambiadas(_,_,_,_,Duenho) <-
				-tablero(celda(X,Y,DuenhoO),_);//Primero se elimina la ficha seleccionada, luego las otras fichas afectadas en caso de ser necesario
				if(not(Duenho==0)){
					+tablero(celda(X,Y,Duenho),e);
				}else{
					+tablero(celda(X,Y,DuenhoO),e);	
				};
...
+!specialExplosion(X,Y,C,T,D,A,P,L) : T = ip  & (D="up" | D="down")&posicionesIntercambiadas(_,_,_,_,Duenho)<-     //Explosion en linea vertical
			-points(L,A,P);
			+points(L,A,P+2);
			for( tablero(celda(X,NY,DuenhoO),ficha(NC,_))){
					!explosion(X,NY);
					/*-tablero(celda(X,NY,DuenhoO),_);
					if(not(Duenho==0)){
						+tablero(celda(X,Y,Duenho),e);
...
+!specialExplosion(X,Y,C,T,D,A,P,L) : T = ip  & (D="left" | D="right")&posicionesIntercambiadas(_,_,_,_,Duenho)<-     //Explosion en linea horizontal
			-points(L,A,P);
			+points(L,A,P+2);
			for( tablero(celda(NX,Y,DuenhoO),ficha(NC,_))){
				 	!explosion(NX,Y);
					/*-tablero(celda(NX,Y,DuenhoO),_);
					if(not(Duenho==0)){
						+tablero(celda(X,Y,Duenho),e);
...
+!specialExplosion(X,Y,C,T,D,A,P,L) : T = gs & tablero(celda(NX,NY,_),ficha(C,_)) & not X=NX & not Y=NY &posicionesIntercambiadas(_,_,_,_,Duenho)<-	 //Explosion de una ficha del mismo color (distinta a si misma)
			-points(L,A,P);
			+points(L,A,P+4);
			!explosion(NX,NY);/*
			-tablero(celda(NX,NY,DuenhoO),ficha(C,_));
			if(not(Duenho==0)){
					+tablero(celda(X,Y,Duenho),e);
				}else{
					+tablero(celda(X,Y,DuenhoO),e);
...
+!coExplosion(X,Y) : tablero(celda(X,Y,_),ficha(C,_)) &posicionesIntercambiadas(_,_,_,_,Duenho)<-
			  !explosion(X,Y);
			  /*
			  -tablero(celda(X,Y,DuenhoO),_);
			  if(not(Duenho==0)){
					+tablero(celda(X,Y,Duenho),e);
				}else{
					+tablero(celda(X,Y,DuenhoO),e);
...
+!specialExplosion(X,Y,C,T,D,A,P,L) : T = ct &posicionesIntercambiadas(_,_,_,_,Duenho)<-     //Explosion de todas las fichas de ese color
			-points(L,A,P);
			+points(L,A,P+8);
			for( tablero(celda(NX,NY,_),ficha(C,_))){
				 	!explosion(NX,NY);/*
					-tablero(celda(NX,NY,DuenhoO),_);
					if(not(Duenho==0)){
						+tablero(celda(X,Y,Duenho),e);
					}else{
						+tablero(celda(X,Y,DuenhoO),e);
...
+!compruebaTerritorios<-
			-territorio(player1,_);
			-territorio(player2,_);
			+territorio(player1,0);
			+territorio(player2,0);
			for( tablero(celda(X,Y,Own),_)){
				if(Own==1){
				-territorio(player1,Celdas);
				+territorio(player1,Celdas+1);
				}
				if(Own==2){
				-territorio(player2,Celdas);
				+territorio(player2,Celdas+1);
				}
			}.
