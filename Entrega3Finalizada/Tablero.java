...
public class Tablero extends Environment {
  ...
  String [][] celdaOwner = new String[GSize][GSize];
  @Override
  ...
  @Override//hay que retocarlo para la et3
  public boolean executeAction(String agent, Structure message) {
    ...
    try {
      if (agent.equals("judge")) { //Recepcion de un mensaje procedente del juez
        ...
        switch(action){
		  ...
		  int dueno =  (int)((NumberTerm)message.getTerm(3)).solve();
          model.delete(c,x,y,dueno);
          //view.repaint();
          break;
          ...
          //view.repaint()
          break;
		  case("repaint"): //Restablecer tablero con la distribucion inicial
          view.repaint();
          break;
          ...
          break;
		  case("putDuenho"): //Establecer nuevo dueño
          x = (int)((NumberTerm)message.getTerm(0)).solve();
          y = (int)((NumberTerm)message.getTerm(1)).solve();
		  int duenho = (int)((NumberTerm)message.getTerm(2)).solve();
          model.putDuenho(x,y,duenho);
          break;
        }
    ...
    class TableroModel extends GridWorldModel {
      ...
      void delete(int color,int x, int y,int dueno) throws Exception { //Borrado de ficha
        steakType[x][y]="";
		if(dueno !=0) celdaOwner[x][y] = Integer.toString(dueno);
	  ...
	  void putDuenho(int x, int y,int duenho){ //establecer duenho
        celdaOwner[x][y]=Integer.toString(duenho);
      }
    ...
    class TableroView extends GridWorldView {
      ...
      public TableroView(TableroModel model) {
        ...
        defaultFont = new Font("Arial", Font.BOLD, 12); //Modificar parï¿½metros para una mejor visualizacion
        setVisible(true);
        repaint();
      }

      //Mï¿½todos de generacion visual de fichas y obstaculos
      @Override//hay que retocar para la et3
      public void draw(Graphics g, int x, int y, int object) {
		  int codColor= Integer.parseInt(celdaOwner[x][y]);
		  if (codColor == 0){
