...
mejorMovTerritorio(0,0,"right",0).
mejorMovTerritorioNeutro(0,0,"right").
...
+!eligeMov<- !actuVirtual;
			-+puntos(0);
			!agrupacionRandom;
			.findall(pos(Xx,Yy),(tableroVirtual(celda(Xx,Yy,_),Ficha) & not(Ficha==obstacle) & not(Ficha==e)),ResulVirtual);
...
//si no hay mov para territorio nuestro ni neutro	
+!seleccionEstrategia:level(L)& L=3 &mejorMovTerritorio(0,0,"right",0) & mejorMovTerritorioNeutro(0,0,"right")<-
			?movRandom(Xenv,Yenv,Direnv,_);
			.print("Envio el movimiento aleatorio ",Xenv," ",Yenv," ",Direnv,", no hay disponible un movimiento mejor");
			.send(judge,tell,moverDesdeEnDireccion(pos(Xenv,Yenv),Direnv));
			-mejorMovTerritorioNeutro(_,_,_);
			+mejorMovTerritorioNeutro(0,0,"right");
			-mejorMovTerritorio(_,_,_,_);
			+mejorMovTerritorio(0,0,"right",0);
			-mejorMovPuntos(_,_,_,_);
			+mejorMovPuntos(0,0,"right",0).

			
//si no hay mov para territorio nuestro pero si neutro
+!seleccionEstrategia:level(L)& L=3&mejorMovTerritorio(0,0,"right",0) & mejorMovTerritorioNeutro(Xenv,Yenv,Direnv)<-
			.print("Envio el movimiento ",Xenv," ",Yenv," ",Direnv,", me da puntos y el territorio no varia");
			.send(judge,tell,moverDesdeEnDireccion(pos(Xenv,Yenv),Direnv));
			-mejorMovTerritorioNeutro(_,_,_);
			+mejorMovTerritorioNeutro(0,0,"right");
			-mejorMovTerritorio(_,_,_,_);
...
// hay mov para territorio nuestro 
+!seleccionEstrategia:level(L)& L=3 & mejorMovTerritorio(Xenv,Yenv,Direnv,_)<-
			.print("Envio el movimiento ",Xenv," ",Yenv," ",Direnv,", me da mas territorio");
			.send(judge,tell,moverDesdeEnDireccion(pos(Xenv,Yenv),Direnv));
			-mejorMovTerritorioNeutro(_,_,_);
			+mejorMovTerritorioNeutro(0,0,"right");
			-mejorMovTerritorio(_,_,_,_);
...
+!agrupacion(X,Y,Dir):plotioTAR(X,Y)<- -+calculaPuntos(X,Y);
										...
										-+calculaTerritorio(X,Y-1);
										-+calculaTerritorio(X,Y-2);
										-+calculaTerritorio(X-1,Y);
										-+calculaTerritorio(X+1,Y);
...
+calculaTerritorio(X,Y):tableroVirtual(celda(X,Y,Own),_)<- 
								.my_name(Nombre);
								?yoSoy(Nombre,Yo);
								if (not Own==Yo) {
								-territorio(Celdas);
...
+guardadoMovimiento(X,Y,Dir)<-
					...
					?mejorMovTerritorio(XmejorTer,YmejorTer,DirmejorTer,NumCeldasMejor);
					?mejorMovTerritorioNeutro(XmejorTerNeu,YmejorTerNeu,DirmejorTerNeu);
					?puntos(PuntosActual);
					?territorio(CeldasActual);
					.my_name(Nombre);
					?yoSoy(Nombre,Yo);
					?tableroVirtual(celda(X,Y,D),_);
						if(PuntosActual>PuntosMejor){
							-mejorMovPuntos(Xmejor,Ymejor,DirMejor,PuntosMejor);
							+mejorMovPuntos(X,Y,Dir,PuntosActual);
							if(D==0){
								-mejorMovTerritorioNeutro(XmejorTerNeu,YmejorTerNeu,DirmejorTerNeu);
								+mejorMovTerritorioNeutro(X,Y,Dir);
								};
						};
						if(PuntosActual==PuntosMejor){
							if(Y>Ymejor){
						...
						if(D==Yo & NumCeldasMejor<CeldasActual){
							-mejorMovTerritorio(XmejorTer,YmejorTer,DirmejorTer,_);
							+mejorMovTerritorio(X,Y,Dir,CeldasActual);
						}
						.	
